...
# separate out by onset and count correct by onset
    ...
    totalTrials = np.array([len(frame) for frame in [maskOnlyOpto, targetOnlyOpto, maskingOpto, catchOpto]])
    
    hits = np.squeeze(np.array(hits))
    resps = np.squeeze(np.array(resps))
    totals = np.squeeze(np.array(totals))

### also need trials wit no opto === opto onset is 'nan';  can change to be onset +2 from max onset value 
...
# plot the percent correct against the opto onset on the xaxis
    ...
    for num, denom, title in zip([resps, hits], 
        ...
        for i, (al, c, lbl, pt) in enumerate(zip([.3,.5, .8, 1], ['k', 'c', 'k', 'm'],
               ...
               [0.671875  , 0.59375   , 0.58461538, 0.35087719])):   # this needs to be resps over totals for no opto

                ax.plot(optoOnset, num[i]/denom[i], 'o-', color = c, lw=3, alpha=al, label=lbl)
                ax.plot(3, pt, 'o', color=c, alpha=al)

            
#        ax.plot(optoOnset, num[3], denom[3], 'mo-', lw=3, alpha=.5, label='Catch')
#                
...
#        xticks = optoOnset.concat(np.round(np.diff(optoOnset)))
        
        ax.set_xticks(optoOnset)
...
#        ax.set_xticklabels(xticklabels)
        ...
        plt.subplots_adjust(top=0.815, bottom=0.105, left=0.1, right=0.92, hspace=0.2, wspace=0.2)
